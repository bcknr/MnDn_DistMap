...
map <- leaflet(options = leafletOptions(minZoom = 7.3)) %>% addTiles() %>%
  ...
  addRasterImage(mn, colors = pal.mn, opacity = 0.7, group = "M. nuda") %>%
  addRasterImage(dn, colors = pal.dn, opacity = 0.7, group = "D. novaeangliae") %>% 
  ...
  addLayersControl(
    ...
    position = "bottomleft") %>% 
  addControlGPS(options = gpsOptions(position = "bottomright", activate = TRUE, 
